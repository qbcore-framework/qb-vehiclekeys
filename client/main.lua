...
local KeysList = {}
local PlayerData = {}
local isTakingKeys, isCarjacking, AlertSend, IsHotwiring, looped = false, false, false, false, false
local canCarjack, trunkclose = true, true
local lastPickedVehicle
...
local function ToggleVehicleLocks(veh)
    ...
    if isBlacklistedVehicle(veh) then
        return TriggerServerEvent('qb-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
    end
...
local function ToggleVehicleunLocks(veh)
    if veh == 0 or veh == nil then return end
...
local function ToggleVehicleTrunk(veh)
    if veh == 0 or veh == nil then return end
...
local function IsBlacklistedWeapon()
    ...
    if weapon and Config.NoCarjackWeapons[weapon] then
        return true
    end
    return false
end
...
local function ToggleVehicleLockswithoutnui(veh)
    if veh == 0 or veh == nil then return end
...
local function robKeyLoop()
    ...
    while looped do
        ...
        if Config.CarJackEnable and canCarjack then
            ...
            if aiming and target ~= nil and target ~= 0 and DoesEntityExist(target) and IsPedInAnyVehicle(target, false) and not IsEntityDead(target) and not IsPedAPlayer(target) then
...
RegisterNetEvent('weapons:client:DrawWeapon', function()
    local ped = PlayerPedId()
    while GetSelectedPedWeapon(ped) == `WEAPON_UNARMED` do
        Wait(1000)
    end
    robKeyLoop()
end)
